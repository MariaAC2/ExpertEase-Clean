...
public class PhotoService(IRepository<WebAppDatabaseContext> repository, IFirebaseRepository firebaseRepository, IFirebaseStorageService firebaseStorageService): IPhotoService
{
    private async Task<string> AddPhoto(PhotoAddDTO photo, CancellationToken cancellationToken = default)
    {
        ...
        await firebaseRepository.AddAsync("photos", photoEntity, cancellationToken);
        return photoEntity.Url;
    }

    private async Task<ServiceResponse> DeletePhoto(string id, CancellationToken cancellationToken = default)
    {
        var photo = await firebaseRepository.GetAsync<Photo>("photos", id, cancellationToken);
        if (photo == null)
        {
        ...
        await firebaseRepository.DeleteAsync<Photo>("photos", id, cancellationToken);

        return ServiceResponse.CreateSuccessResponse();
    }

    public async Task<ServiceResponse> AddProfilePicture(ProfilePictureAddDTO photo, UserDTO? requestingUser = null,
    ...
    public async Task<ServiceResponse> AddPortfolioPicture(PortfolioPictureAddDTO photo, UserDTO? requestingUser = null,
        CancellationToken cancellationToken = default)
    {
        if (requestingUser == null)
        {
    ...
    public async Task<ServiceResponse> UpdateProfilePicture(ProfilePictureAddDTO photo, UserDTO? requestingUser = null, CancellationToken cancellationToken = default)
    {
        ...
        var existingPhotos = await firebaseRepository.ListAsync<Photo>(
            "photos",
    ...
    {
        var photo = await firebaseRepository.GetAsync<Photo>("photos", photoId, cancellationToken);
        ...
        await firebaseRepository.DeleteAsync<Photo>("photos", photoId, cancellationToken);
