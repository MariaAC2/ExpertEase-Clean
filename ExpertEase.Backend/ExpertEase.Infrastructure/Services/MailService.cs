...
public class MailService(IOptions<MailConfiguration> mailConfiguration) : IMailService
{
        ...
        string? senderTitle = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            using var client = new MailKit.Net.Smtp.SmtpClient(); // Create the SMTP client. Note that this object is disposable and as such need to use the keyword "using" to properly dispose the object after leaving the scope.
        ...
        {
            return ServiceResponse.CreateErrorResponse(new(HttpStatusCode.ServiceUnavailable, "Mail couldn't be send!", ErrorCodes.MailSendFailed));
        }
